...
JOIN - объединение строк из нескольких связанных таблиц по горизонтали запросами: inner join, left join, right join, full outer join, cross join
[UNION|INTERSECT|EXCEPT] - объединение строк по вертикали(убирает дубли), пересечение строк(оставляет только присутствующие в результатах обоих запросов), 
...
-- Доп.операторы условий: AND,OR,[NOT]IN,[NOT]BETWEEN,[NOT]LIKE, ILIKE(игнорируем регистр), IS [NOT]NULL.  
...
ORDER BY - сортирует результирующую таблицу [ASC | DESC] [NULLS FIRST | NULLS LAST],  сортировку можно выполнить по каждому полю 
LIMIT - позволяет извлечь определенное кол-во строк. Используем OFFSSET для получения верхних/нижних N строк примимо 

-- агрегатные функции вытаскивают значения в новое поле
-- фун-ия count(columns) не считает NULL строки 
...
-- Оконные функции - нужны тогда, когда нужно получить в текущей таблице, значение соседних строк не меняя группировки, т.е работают со строками или окном. 
-- запросы ORDER BY, LIMIT выполняются после окнной функции, все остальные запросы выполняется перед оконкой. 
...
-- т.е применили оконную функцию и посчитали сумму всех оплат(100% поля orderSum), при этом в результатирующей таблице сохранилась первая группировка.
     ...
     user_id,
          ...
          sum(sum(order_sum)) OVER () AS allOrderSum -- вызываем оконую функцию и задаем алиас, здесь мы не указываем PARTITION BY, это значит что у нашего окна начало и конец окна не определены, и по дефолту выбераем все строки.
     FROM `chint-297212.test_dataset1.e1_orders - orders`
     GROUP BY 1
)
...
PARTITION BY -- работает так же как и GROUP BY только в рамках окна. Применяет функцию которую мы указали для оконки в рамках группировки.
ORDER BY -- сортировка в рамках окна. ORDER BY без параметров "ROWS"- накапливает значения, т.е мы указываем сортировку по колонке и как бы в рамках этой колонке формируется группировка и каждые последующие значения включают в себя предыдущие значения. ORDER BY влияет на значения которое рассчитает окно т.к зависит в каком порядке значения будут выстроенны, в таком порядке и будут вычисленны значения по функции агрегации которую мы указали для оконки. 
...
PRECEDING -- «Значение» определяет число строк перед текущей строкой. Например " 2 PRECEDING" - это взять значение текущей строки и две верхних.
FOLLOWING -- «Значение» определяет число строк после текущей строки. Например " 1 PRECEDING" - это взять значение текущей строки и одну нижнию.
-- Когда работаем с PRECEDING и FOLLOWING значение текущей строки учитывается один раз.


SELECT
  order_date,
  order_sum
  ,sum(order_sum) OVER win_part AS allOrderSum --вызываем оконную функцию, прописываем ей имя, и дальше уже будем с ней работать в блоке FROM или после WHERE. В селекте, мы можем использовать более 1 раза именованную функцию.  
FROM `chint-297212.test_dataset1.e1_orders - orders`
...
-- или функция DATE()
     ...
     UNNEST(event_params) AS param
     LIMIT 100 -- можем передавать два число с помощью которых будет идти смещение(пропускать строки) или указать оператор OFFSSET 5 - этот процесс называют пагинацией
...
-- чтобы сделать запрос экономичным, будем передавать константное значение, т.к оно ни когда не принимает NULL и вернет кол-во строк. 
-- как это работает - выбирая из таблицы мы не используем промежуточную память, а для каждой из строк выполняем фун-ию count(), которой вообще не нужны какие то значения из таблицы, она просто делает +1 к каждой строке. 
SELECT COUNT(2) 
FROM `table`